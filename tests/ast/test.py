if __name__ == '__main__':
    ...
    def visualize_ast(node, graph, parent_name, my_name):
        graph.node(my_name, label=type(node).__name__)
        if parent_name is not None:
            graph.edge(parent_name, my_name)
        for child_name, child in ast.iter_fields(node):
            if isinstance(child, ast.AST):
                child_id = my_name + '.' + child_name
                visualize_ast(child, graph, my_name, child_id)
            elif isinstance(child, list):
                for index, item in enumerate(child):
                    if isinstance(item, ast.AST):
                        item_id = "{}.{}_{}".format(my_name, child_name, index)
                        visualize_ast(item, graph, my_name, item_id)


    def ast_visualization(source_code):
        tree = ast.parse(source_code)
        graph = Digraph()
        visualize_ast(tree, graph, None, 'root')
        return graph
...
"""
    ...
    graph = ast_visualization(source_code)
    graph.view()  # This will generate and open the graph in a viewer.
